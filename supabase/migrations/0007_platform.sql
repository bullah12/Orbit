...
select app.apply_standard_rls('activity_log');
select app.apply_standard_rls('ai_runs');
select app.apply_standard_rls('encrypted_blobs');
...
-- ===========================================================================
create or replace function app.space_move_preview(
...
begin
  ...
  if not app.can_read_space(v_source_space) then
    raise exception 'space_move_preview: not a member of the source space'
      using errcode = 'insufficient_privilege';
  end if;

  if not app.can_write_space(p_target_space_id) then
...
grant execute on function app.space_move_preview(app.entity_kind, uuid, uuid) to authenticated;
