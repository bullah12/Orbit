...
create index events_space_starts_idx on public.events (space_id, starts_at);
create index events_range_idx on public.events (space_id, starts_at, ends_at)
  where status <> 'cancelled';
create index events_dirty_idx on public.events (space_id) where is_dirty;
...
-- ---------------------------------------------------------------------------
create table public.free_busy_shares (
  id           uuid primary key default gen_random_uuid(),
  space_id     uuid not null references public.spaces(id) on delete cascade,
  owner_id     uuid not null references public.profiles(id) on delete cascade,
...
create trigger free_busy_shares_touch before update on public.free_busy_shares
  for each row execute function app.touch_updated_at();
...
alter table public.free_busy_shares enable row level security;
grant select, insert, update, delete on public.free_busy_shares to authenticated;

create policy free_busy_shares_select on public.free_busy_shares for select to authenticated
using (app.can_read_space(space_id) or grantee_id = auth.uid());

create policy free_busy_shares_insert on public.free_busy_shares for insert to authenticated
with check (app.is_space_admin(space_id) and owner_id = auth.uid());

create policy free_busy_shares_update on public.free_busy_shares for update to authenticated
using (app.is_space_admin(space_id)) with check (app.is_space_admin(space_id));

create policy free_busy_shares_delete on public.free_busy_shares for delete to authenticated
