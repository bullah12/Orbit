...
-- RLS
-- ===========================================================================
select orbit.apply_standard_rls('calendar_accounts');
select orbit.apply_standard_rls('calendars');
select orbit.apply_standard_rls('events', p_has_visibility => true);
select orbit.apply_standard_rls('event_attendees');
select orbit.apply_standard_rls('calendar_sync_state');
...
using (orbit.is_space_admin(space_id) or grantee_id = auth.uid());

-- ---------------------------------------------------------------------------
-- orbit.free_busy_blocks() — the ONLY way a free_busy participant reaches event
...
as $$
  ...
  where e.space_id = p_space_id
    ...
    and (
      orbit.can_read_space(p_space_id)
      or exists (
