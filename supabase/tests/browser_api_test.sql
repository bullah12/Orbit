...
insert into orbit.places (id, space_id, owner_id, name, geom) values
  ('aaaaaaaa-2000-0000-0000-000000000001', 'aaaaaaaa-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', 'Alice home', public.ST_SetSRID(public.ST_MakePoint(-1.9, 52.48), 4326)::public.geography);
insert into orbit.people (id, space_id, owner_id, display_name, home_place_id) values
  ('aaaaaaaa-3000-0000-0000-000000000001', 'aaaaaaaa-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', 'Visible friend', 'aaaaaaaa-2000-0000-0000-000000000001');
insert into orbit.tags (id, space_id, owner_id, name, slug) values
  ('aaaaaaaa-4000-0000-0000-000000000001', 'aaaaaaaa-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', 'Friends', 'friends');
insert into orbit.taggings (space_id, owner_id, tag_id, entity_kind, entity_id) values
  ('aaaaaaaa-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', 'aaaaaaaa-4000-0000-0000-000000000001', 'person', 'aaaaaaaa-3000-0000-0000-000000000001');
...
select is(jsonb_array_length(orbit.people_directory()), 1, 'people directory returns readable people with map and tag data');
...
select is(jsonb_array_length(orbit.people_directory()), 0, 'outsider cannot widen people directory results');
...
select is((select count(*)::int from information_schema.routine_privileges where specific_schema = 'orbit' and routine_name in ('ensure_account', 'ensure_default_spaces', 'create_space', 'invite_preview', 'invite_accept', 'invite_decline', 'space_move_preview', 'free_busy_blocks', 'free_busy_recurring', 'dashboard', 'search', 'people_directory') and grantee = 'PUBLIC' and privilege_type = 'EXECUTE'), 0, 'PUBLIC can execute no browser wrapper');
select is((select count(*)::int from information_schema.routine_privileges where specific_schema = 'orbit' and routine_name in ('ensure_account', 'ensure_default_spaces', 'create_space', 'invite_preview', 'invite_accept', 'invite_decline', 'space_move_preview', 'free_busy_blocks', 'free_busy_recurring', 'dashboard', 'search', 'people_directory') and grantee = 'anon' and privilege_type = 'EXECUTE'), 0, 'anonymous users can execute no browser wrapper');
select is((select count(*)::int from information_schema.routines where routine_schema = 'orbit' and routine_name in ('ensure_account', 'ensure_default_spaces', 'create_space', 'invite_preview', 'invite_accept', 'invite_decline', 'space_move_preview', 'free_busy_blocks', 'free_busy_recurring', 'dashboard', 'search', 'people_directory') and security_type <> 'INVOKER'), 0, 'every browser wrapper is SECURITY INVOKER');
select is((select count(*)::int from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'orbit' and p.proname in ('ensure_account', 'ensure_default_spaces', 'create_space', 'invite_preview', 'invite_accept', 'invite_decline', 'space_move_preview', 'free_busy_blocks', 'free_busy_recurring', 'dashboard', 'search', 'people_directory') and array_to_string(p.proconfig, ',') like '%public%'), 0, 'browser wrapper search paths contain no public schema');
select is((select count(*)::int from information_schema.routines where routine_schema = 'orbit' and routine_name in ('ensure_account', 'ensure_default_spaces', 'create_space', 'invite_preview', 'invite_accept', 'invite_decline', 'space_move_preview', 'free_busy_blocks', 'free_busy_recurring', 'dashboard', 'search', 'people_directory')), 12, 'orbit exposes exactly the twelve reviewed browser wrappers');
