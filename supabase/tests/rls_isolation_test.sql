...
-- Orbit's tables and helpers are all in the `orbit` schema. Every reference
-- below names it anyway; the search_path is here so that pgTAP's own functions
-- (which live wherever the extension was installed) and Orbit's resolve in the
-- same session.
...
-- Explicitly into `public`: with `orbit` first on the search_path an
-- unqualified CREATE EXTENSION would install pgTAP's several hundred functions
-- into Orbit's own schema, and a test harness is not Orbit's to own.
...
-- participant gets times from orbit.free_busy_blocks() and nothing else — so
...
select is(
  (select count(*)::int from orbit.free_busy_blocks(
    'aaaaaaaa-0000-0000-0000-000000000002',
    '2026-08-01 00:00+01', '2026-08-05 00:00+01')),
  1,
  'a free_busy participant gets anonymous blocks through orbit.free_busy_blocks()');
...
select is(
  (select count(*)::int from orbit.space_move_preview(
...
select throws_ok(
  $$select * from orbit.space_move_preview(
      'task', 'bbbbbbbb-0000-0000-0000-000000000002',
...
-- ===========================================================================
-- 9b. orbit.entity_space — SECURITY INVOKER, so RLS decides
--
...
select is(
  (select space_id from orbit.entity_space('task', 'bbbbbbbb-0000-0000-0000-000000000002')),
...
select is(
  (select count(*)::int from orbit.entity_space('task', 'bbbbbbbb-0000-0000-0000-000000000002')),
  0,
...
select is(
  (select count(*)::int from orbit.entity_space('task', 'bbbbbbbb-0000-0000-0000-000000000003')),
  0,
...
select is(
  (select count(*)::int
   ...
   where n.id = 'cccccccc-0000-0000-0000-000000000001'
     and exists (select 1 from orbit.entity_space('task', 'bbbbbbbb-0000-0000-0000-000000000001') es
...
-- ===========================================================================
-- 12. Space invites — orbit.space_invite, the one SECURITY DEFINER exception
--
...
-- `sha256` from pg_catalog rather than pgcrypto's `digest`, matching
-- orbit.space_invite() — see the comment there for why an extension function
-- is the wrong dependency for this.
...
select is(
  (select i.status from orbit.space_invite('open-token', 'preview') i),
  'ok',
  'but the function shows her which space and which role the link is for');

select is(
  (select i.space_name from orbit.space_invite('open-token', 'preview') i),
  'Home',
  'named, so nobody accepts an invitation to a space they cannot see');

select is(
  (select i.status from orbit.space_invite('no-such-token', 'preview') i),
  'unknown',
  'a token nobody issued is "unknown" — a sentence, not an error and not a hint that a space exists');

select is(
  (select i.status from orbit.space_invite('stale-token', 'accept') i),
  'expired',
  'an expired invite is refused by name, whatever verb is asked for');

select is(
  (select i.status from orbit.space_invite('for-carol', 'accept') i),
  'wrong_person',
...
select is(
  (select i.status from orbit.space_invite('open-token', 'accept') i),
  'accepted',
