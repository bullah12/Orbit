...
// Every statement below names the `orbit` schema. The connection option is for
// what a statement cannot name: a type looked up by name, PostGIS. `public` and
// `extensions` follow it because an installation puts PostGIS in one or the
// other. Same string as src/lib/db/index.ts, for the same reason.
const sql = postgres(SEED_DATABASE_URL, {
  max: 4,
  onnotice: () => {},
  connection: { search_path: 'orbit, public, extensions' },
});
    ...
    for (const d of devices) {
      for (const kind of ['task', 'note', 'event', 'person', 'place'] as const) {
        ...
        await sql`
          ...
          values (${d.space_id}, ${d.owner_id}, ${d.id}, ${kind}::orbit.entity_kind,
