...
export default function CalendarEventEditor({ id, date, onClose }: { id: string | null; date: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (id) {
      void supabase.from('events').select('*, recurrence_rules:recurrence_rule_id(*)').eq('id', id).single()
        .then(({ data, error: loadError }) => loadError ? setError(loadError.message) : setRecord(data as unknown as Event));
    }
  }, [id]);
  ...
  return <dialog ref={dialog} className={s.dialog} onClose={onClose} onCancel={onClose}>
    <form onSubmit={async (event) => {
      ...
      const starts = new Date(String(data.get('starts_at')));
      const ends = new Date(String(data.get('ends_at')));
      try {
        const changes = {
          title: String(data.get('title')),
          body_md: String(data.get('body_md')),
          space_id: id ? record!.space_id : String(data.get('space_id')),
      ...
      <div className={`${s.dialogBody} ${s.form}`}>
        ...
        {id && !record && !error ? <p role="status">Loading event…</p> : record?.is_locked ? <p className={s.error}>Locked events cannot be edited in this release.</p> : <>
          ...
          <div className={s.field}><label htmlFor="event-notes">Notes</label><textarea id="event-notes" className={s.textarea} name="body_md" defaultValue={record?.body_md} /></div>
        </>}
        {error && <div className={s.error}>{error}</div>}
      </div>
      <div className={s.dialogActions}><button type="button" className={s.secondaryButton} onClick={onClose}>Cancel</button><button className={s.primaryButton} disabled={saving || record?.is_locked || Boolean(id && !record)}>{saving ? 'Saving…' : 'Save event'}</button></div>
    </form>
  </dialog>;
}
